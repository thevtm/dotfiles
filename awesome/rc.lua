...
-- {{{ Menu
local my_awesome_menu = require("menu").build(awesome)
-- }}}
...
-- Calendar
local my_calendar = require("widgets.calendar").build(my_clock)

-- ALSA volume
local volume = require("widgets.volume")
local my_volume = volume.build()

-- Memory
local memory = require("widgets.memory")
local my_memory = memory.build()
...
function beautiful.at_screen_connect(s)
	...
	s.quake_top = quake.top.build()
	s.quake_bottom = quake.bottom.build()
	...
	s.mywibox:setup {
		...
		{ -- Right widgets
			...
			arrow(beautiful.bg_normal, cpu.bg_color),
			wibox.container.background(wibox.container.margin(wibox.widget { cpu.icon, my_cpu.widget, layout = wibox.layout.align.horizontal }, 3, 4), cpu.bg_color),
...
globalkeys = awful.util.table.join(
	...
	awful.key({ modkey,           }, "z", function () awful.screen.focused().quake_top:toggle() end),
	awful.key({ modkey, "Shift"   }, "z", function () awful.screen.focused().quake_bottom:toggle() end),
...
awful.rules.rules = {
	...
	{ rule = { },
		except = { name = "QuakeTermTop" },
		except = { name = "QuakeTermBottom" },
